...
const buildDefaultText = (): string => {
  const sha = process.env.GITHUB_SHA ?? "";
  const shortSha = sha ? sha.substring(0, 7) : "";
  const githubServer = process.env.GITHUB_SERVER_URL;
  const repo = process.env.GITHUB_REPOSITORY;
  const commitUrl = sha && githubServer && repo ? `${githubServer}/${repo}/commit/${sha}` : undefined;
  const prUrl = process.env[ENV_PR_URL];
  const branchLine = process.env[ENV_BRANCH_NAME] ? `Branch: ${process.env[ENV_BRANCH_NAME]}` : undefined;
  const mrLine = prUrl ? `MR: [${prUrl}](${prUrl})` : undefined;
  const commitLine = shortSha ? (commitUrl ? `Commit: [${shortSha}](${commitUrl})` : `commit: ${shortSha}`) : undefined;
  return [branchLine, mrLine, commitLine].filter(Boolean).join("\n");
};

const sendText = async (webhook: string, text: string): Promise<void> => {
  const payload = {
  ...
  if (!res.ok) {
    ...
    process.stderr.write(`Failed to send Lark message: ${res.status} ${body}\n`);
    process.exit(1);
  }
};
...
const sendInteractive = async (webhook: string, title: string, text: string): Promise<void> => {
  ...
  try {
    ...
    const projectName = getEnv(ENV_CF_PAGES_PROJECT) ?? "plaud-web3";
  ...
  const linkLines = [previewLine, aliasLine].filter(Boolean).join("\n");
  ...
  const payload = {
    ...
    card: {
      ...
      elements: [
        ...(actor
          ? [{ tag: "div", text: { tag: "lark_md", content: actorUrl ? `Triggered by: [${actor}](${actorUrl})` : `Triggered by: ${actor}` } }]
          : []),
